...
-   Các phương pháp **Regression-based**: đại khái là sau khi `bounding box` được dự đoán, `bounding box` cuối cùng sẽ được lọc qua bằng thuật toán **Non-Maximum Suppression**. Phần lớn `bounding box` của phương pháp này chỉ có 4 điểm tọa độ nên phương pháp này sẽ bị giới hạn trong việc biểu diễn các `bounding box` hay các văn bản có hình dạng bất thường (ví dụ như bị cong) trong ảnh. Một số mô hình nổi bật cho phương pháp này có thể kể đến như [EAST](https://github.com/argman/EAST) hay [TextBoxes](https://github.com/MhLiao/TextBoxes).
